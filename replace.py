...
pattern = re.compile(r'<section className="section-teal py-24 bg-\[#0a1e1e\]">.*?</section>', re.DOTALL)

new_str = """<section className="section-teal py-24 bg-[#F1EFE1]">
        <div className="container-wispr text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {[
              ...
              <div key={i} className="bg-white border border-black/5 rounded-3xl p-8 text-left shadow-sm relative overflow-hidden">
                <div className="text-5xl font-black text-[#222222]/5 absolute -top-4 -right-2">{s.step}</div>
                <div className="w-10 h-10 rounded-full bg-[#00F0FF]/10 text-[#00F0FF] flex items-center justify-center font-extrabold mb-6">
                  {s.step}
                </div>
